...
from drl_grasping.utils.training import linear_schedule
...
def sample_sac_params(trial: optuna.Trial) -> Dict[str, Any]:
    ...
    # High should be equal to the optimization n_timestamps. Higher than that makes no sense
    # buffer_size = trial.suggest_int(
    #     "buffer_size", low=5000, high=25000, log=True)
    buffer_size = 25000
    # learning_starts = trial.suggest_int(
    #     "learning_starts", low=500, high=100*(buffer_size//2//100), step=100)
    learning_starts = 5000

    batch_size = trial.suggest_categorical(
        "batch_size", [16, 32, 64])
    learning_rate = trial.suggest_loguniform(
        "learning_rate", low=0.00001, high=0.01)

    gamma = trial.suggest_loguniform("gamma", low=0.975, high=1.0)
    tau = trial.suggest_loguniform("tau", low=0.005, high=0.05)
