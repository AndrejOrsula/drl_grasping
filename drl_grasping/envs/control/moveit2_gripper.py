...
class MoveIt2GripperCommand:
    ...
    def open(self):
        ...
        # Don't do anything if open already or if interruptions are not desired
        if self.is_open:
            return
        if self.__ignore_new_calls_while_executing:
    ...
    def close(self):
        ...
        # Don't do anything if closed already or if interruptions are not desired
        if self.is_closed:
            return
        if self.__ignore_new_calls_while_executing:
            if self.__is_executing:
    ...
    def reset_open(self):
        """
        Reset controller to an open state.
        """
    ...
    def reset_closed(self):
        """
        Reset controller to a closed state.
        """
...
class MoveIt2GripperTrajectory:
    def __init__(
        self,
        node: Node,
        frame_id: str,
...
class MoveIt2Gripper(MoveIt2GripperTrajectory):
    def __init__(
        self,
        **kwargs,
    ):
