...
@enum.unique
class GraspPlanetaryStage(enum.Enum):
    """
    ...
    @classmethod
    def first(self) -> Type[GraspPlanetaryStage]:

        return GraspPlanetaryStage(1)

    @classmethod
    def last(self) -> Type[GraspPlanetaryStage]:

        return GraspPlanetaryStage(len(GraspPlanetaryStage))

    def next(self) -> Union[Type[GraspPlanetaryStage], None]:
        ...
        if next_value > self.last().value:
            ...
            return GraspPlanetaryStage(next_value)

    def previous(self) -> Union[Type[GraspPlanetaryStage], None]:
        ...
        if previous_value < self.first().value:
            ...
            return GraspPlanetaryStage(previous_value)


class GraspPlanetaryCurriculum:
    """
    ...
    def __init__(
        ...
        self._outside_workspace_reward = (
            outside_workspace_reward
            if outside_workspace_reward >= 0.0
            else -outside_workspace_reward
        )
        self._ground_collision_reward = (
            ground_collision_reward
            if ground_collision_reward >= 0.0
            else -ground_collision_reward
        )
        ...
        if self._stage_increase_rewards and self._stage_reward_multiplier < 1.0:
            raise Exception(
                "GraspPlanetaryCurriculum: Stage reward multiplier must be >= 1.0 "
        ...
        if not self._enable_stages:
            self._stage: GraspPlanetaryStage = GraspPlanetaryStage.last()
        elif not self._skip_reach_stage:
            self._stage: GraspPlanetaryStage = GraspPlanetaryStage.REACH
        else:
            self._stage: GraspPlanetaryStage = GraspPlanetaryStage.TOUCH
        # Dict of bools determining if stage has been completed in the current episode (dict excludes the last stage)
        self._stage_completed: Dict[GraspPlanetaryStage, bool] = {
            GraspPlanetaryStage(stage): False
            for stage in range(
                GraspPlanetaryStage.first().value, GraspPlanetaryStage.last().value + 1
            )
        }
        ...
        self._remaining_dense_reward_REACH = self._stage_reward_multiplier ** (
            GraspPlanetaryStage.REACH.value - 1
        )
        self._remaining_dense_reward_LIFT = self._stage_reward_multiplier ** (
            GraspPlanetaryStage.LIFT.value - 1
        )
        ...
        if self._normalize_reward:
            self._normalize_positive_reward_multiplier = 1.0 / sum(
                [
                    self._stage_reward_multiplier ** i
                    for i in range(len(GraspPlanetaryStage))
                ]
            )

            if (
                0 == self._ground_collision_reward
                ...
                max_negative_reward = (
                    self._max_episode_length * self._act_quick_reward
                ) + max(
                    self._n_ground_collisions_till_termination
                    * self._ground_collision_reward,
                    self._outside_workspace_reward,
    ...
    def update_success_rate(self, is_success: bool):
        ...
        self._success_rate = (
            (self._success_rate_rolling_average_n - 1) * self._success_rate
            + float(is_success)
        ...
        if self._enable_object_count_increase:
            extra_objects = math.floor(
                (self._max_object_count - 1)
                * (self._success_rate / self._success_rate_threshold)
            )
            self.task.object_count_override = min(
                1 + extra_objects, self._max_object_count
            )
    ...
    def maybe_next_stage(self) -> bool:
        ...
        if GraspPlanetaryStage.last() == self._stage:
            return False
        ...
        if self._success_rate > self._success_rate_threshold:
            ...
            if self._skip_grasp_stage and GraspPlanetaryStage.GRASP == next_stage:
                print(
                    f"Skipping {GraspPlanetaryStage.GRASP} stage and "
    ...
    def maybe_restart_to_first_stage(self) -> bool:
        ...
        if self._reset_step_counter <= 0:
            print(
                "Curriculum stage change:\n"
                f"\tRestarting to first stage ({GraspPlanetaryStage.first()}) after {self._restart_every_n_steps} episodes "
                f"have elapsed since reaching the second stage ({GraspPlanetaryStage(GraspPlanetaryStage.first().value+1)}). "
                f"Success rate reset to {0.5*self._success_rate_threshold}."
            )
            self._stage = GraspPlanetaryStage.first()
    ...
    def get_reward(self) -> float:
        ...
        first_stage_to_process = GraspPlanetaryStage.last().value
        for stage in range(
            GraspPlanetaryStage.first().value, GraspPlanetaryStage.last().value
        ):
            if not self._stage_completed[GraspPlanetaryStage(stage)]:
        ...
        for stage in range(
            first_stage_to_process, GraspPlanetaryStage.last().value + 1
        ):

            # grasped_objects are needed for GRASP and LIFT, add once reached
            if (
                stage >= GraspPlanetaryStage.GRASP.value
                and not "grasped_objects" in kwargs
            ):
            ...
            reward += reward_multiplier * self.GET_REWARD[GraspPlanetaryStage(stage)](
                self, **kwargs
            )
    ...
    def get_info(self) -> Dict:
        ...
        if (
            self._stage != GraspPlanetaryStage.first()
            and self._restart_every_n_steps > 0
        ):
            self._reset_step_counter -= 1
            self.maybe_restart_to_first_stage()

        info = {
            "is_success": self._stage_completed[GraspPlanetaryStage.last()],
    ...
    def reset_task(self):
        ...
        for stage in range(
            ...
            self._stage_completed[GraspPlanetaryStage(stage)] = False
    ...
    def _get_reward_REACH(self, **kwargs) -> float:
        ...
        current_min_distance = self.task.get_closest_object_distance(object_positions)
        if current_min_distance < self._required_reach_distance:
            self._is_success = (
                True
                if GraspPlanetaryStage.REACH.value >= self._stage.value
                else self._is_success
            )
            self._stage_completed[GraspPlanetaryStage.REACH] = True
    ...
    def _get_reward_TOUCH(self, **kwargs) -> float:
        ...
        if len(touched_objects) > 0:
            self._is_success = (
                True
                if GraspPlanetaryStage.TOUCH.value >= self._stage.value
                else self._is_success
            )
            self._stage_completed[GraspPlanetaryStage.TOUCH] = True
            return 1.0
        else:
            return 0.0
    ...
    def _get_reward_GRASP(self, **kwargs) -> float:
        ...
        if len(grasped_objects) > 0:
            self._is_success = (
                True
                if GraspPlanetaryStage.GRASP.value >= self._stage.value
                else self._is_success
            )
            self._stage_completed[GraspPlanetaryStage.GRASP] = True
            if self._verbose:
    ...
    def _get_reward_LIFT(self, **kwargs) -> float:
        ...
        for grasped_object in grasped_objects:
            if object_positions[grasped_object][2] > self._required_lift_height:
                self._is_success = (
                    True
                    if GraspPlanetaryStage.LIFT.value >= self._stage.value
                    else self._is_success
                )
                self._stage_completed[GraspPlanetaryStage.LIFT] = True
                if self._sparse_reward:
                    reward += 1.0

            if not self._sparse_reward:
                height_increase = (
                    object_positions[grasped_object][2]
    ...
    def _log_curriculum(self):
        self.logger.record(
            "curriculum/current_stage", self._stage, exclude="tensorboard"
        )
        self.logger.record(
            "curriculum/current_stage_id", self._stage.value, exclude="stdout"
        )
        self.logger.record("curriculum/current_success_rate", self._success_rate)
        if self._restart_every_n_steps > 0:
            self.logger.record("curriculum/steps_until_reset", self._reset_step_counter)

    GET_REWARD = {
        GraspPlanetaryStage.REACH: _get_reward_REACH,
        GraspPlanetaryStage.TOUCH: _get_reward_TOUCH,
        GraspPlanetaryStage.GRASP: _get_reward_GRASP,
        GraspPlanetaryStage.LIFT: _get_reward_LIFT,
    }
