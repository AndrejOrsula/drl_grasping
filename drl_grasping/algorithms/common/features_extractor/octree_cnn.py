...
class OctreeCnnFeaturesExtractor(BaseFeaturesExtractor):
    ...
    def __init__(self,
        ...
        if not self._separate_networks_for_stacks:
            ...
            self.convs = torch.nn.ModuleList([torch.nn.ModuleList([OctreeConv(depth-i, channels[i], channels[i+1], **bn_kwargs)
                                                                   for i in range(self._n_convs)])
                                              for _ in range(self._n_stacks)])
            self.pools = torch.nn.ModuleList([torch.nn.ModuleList([OctreePool(depth-i)
                                                                   for i in range(self._n_convs)])
                                              for _ in range(self._n_stacks)])
            ...
            if self._full_depth_conv1d:
                # Use 1D convolution (Conv1D instead of linear is used here to preserve spatial information)
                self.full_depth_conv = torch.nn.ModuleList([OctreeConv1D(channels[-1],
                                                                         full_depth_channels,
                                                                         **bn_kwargs)
                                                            for _ in range(self._n_stacks)])
            else:
                # Use 3D convolution (same as previous modules)
                self.full_depth_conv = torch.nn.ModuleList([OctreeConv(full_depth,
                                                                       channels[-1],
                                                                       full_depth_channels,
                                                                       **bn_kwargs)
                                                            for _ in range(self._n_stacks)])
            ...
            # Layer that converts octree at depth=full_depth into a full voxel grid (zero padding) such that it has a fixed size
            self.octree2voxel = torch.nn.ModuleList([ocnn.FullOctree2Voxel(full_depth)
                                                     for _ in range(self._n_stacks)])

            # Layer that flattens the voxel grid into a feature vector
            self.flatten = torch.nn.ModuleList([torch.nn.Flatten()
                                                for _ in range(self._n_stacks)])

            # Last linear layer of the extractor, applied to all (flattened) voxels at full depth
            self.linear = torch.nn.ModuleList([LinearRelu(flatten_dim, features_dim)
                                               for _ in range(self._n_stacks)])
    ...
    def forward(self, octree):
        ...
        if not self._separate_networks_for_stacks:
            ...
            if self._aux_obs_dim != 0:
                ...
                aux_data = self.aux_obs_linear(aux_obs.view(-1,
                                                            self._aux_obs_dim))
            ...
            # Extract features from the octree at the finest depth
            data = [ocnn.octree_property(octree[i], 'feature',
                                         self._depth)
