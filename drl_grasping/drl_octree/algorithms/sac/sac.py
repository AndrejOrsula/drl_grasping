# This module adds a monkey patch to SAC `_store_transition` function - The only difference is that if info contains 'curriculum.restart_exploration' key set to True, the ent_coef and its optimizer will be reset.
...
# Note: Import monkey patch of off_policy_algorithm before SAC
    ...
    if isinstance(self.ent_coef, str) and self.ent_coef.startswith("auto"):
        ...
        if "_" in self.ent_coef:
            ...
            if len(self.ent_coef.split("_")) == 3:
                ...
                assert (
                    restart_value > 0.0
                ), "The retart value of ent_coef must be greater than 0"
...
def _store_transition_allow_exploration_reset(
    ...
    if infos[0].get("curriculum.restart_exploration", False):
        if self.ent_coef_optimizer is not None:
            restart_value = (
                self.ent_coef_restart_value
                if hasattr(self, "ent_coef_restart_value")
                else 1.0
            )
            print(
            ...
            self.log_ent_coef = th.log(
                th.ones(1, device=self.device) * restart_value
            ).requires_grad_(True)
