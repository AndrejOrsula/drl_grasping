...
"""Evaluate an RL agent on a real robot"""
...
def generate_launch_description() -> LaunchDescription:
    ...
    environment_variables = [
        # Use the correct run-time for the task
        SetEnvironmentVariable(name="DRL_GRASPING_REAL_EVALUATION", value="true")
    ]
            ...
            PythonLaunchDescriptionSource(
                PathJoinSubstitution(
                    [
                        ...
                        "launch",
                        "real",
                        "lunalab_summit_xl_gen.launch.py",
                    ]
                )
            ),
            launch_arguments=[
    ...
    nodes = [
        # Evaluation node (lunalab_summit_xl_gen)
        Node(
            ...
            remappings=[
                (
                    "/rgbd_camera/points",
                    "/lunalab_summit_xl_gen_d455/depth/color/points",
                ),
                (
                    "/joint_trajectory_controller/follow_joint_trajectory",
                    "/robot_j2s7s300/follow_joint_trajectory",
                ),
                (
                    "/gripper_trajectory_controller/follow_joint_trajectory",
                    "/robot_j2s7s300_gripper/gripper_command",
                ),
            ],
            condition=LaunchConfigurationEquals("robot_model", "lunalab_summit_xl_gen"),
        ),
    ...
    return LaunchDescription(
        declared_arguments + environment_variables + launch_descriptions + nodes
    )
